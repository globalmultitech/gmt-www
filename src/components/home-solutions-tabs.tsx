    ...
    <section className="py-16 md:py-28 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl md:text-5xl font-headline font-extrabold text-primary">Solusi Kami Lainnya</h2>
        ...
        <Tabs defaultValue={defaultTab} className="w-full">
          <TabsList className="grid w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mx-auto max-w-4xl h-auto flex-wrap">
            {solutionGroups.map((group) => (
              <TabsTrigger key={group.id} value={group.title} className="flex-1 text-xs md:text-sm">{group.title}</TabsTrigger>
            ))}
          </TabsList>
          
          {solutionGroups.map((group) => (
            <TabsContent key={group.id} value={group.title} className="mt-8 md:mt-12">
              <div className="grid md:grid-cols-2 gap-4 md:gap-8">
                {group.children.map((solution) => (
                  <Card key={solution.id} className="p-4 md:p-8 shadow-lg rounded-lg transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl">
                    <div className="flex items-start gap-4 md:gap-6">
                      ...
                      <div className="flex-grow">
                        <h3 className="text-base md:text-xl font-bold text-primary mb-2">{solution.title}</h3>
                        <p className="text-muted-foreground text-xs md:text-sm mb-4 h-16 md:h-20">{solution.description}</p>
                        <Link href={`/solusi/${solution.slug}`} onClick={startLoading} className="font-semibold text-sky-blue flex items-center group text-sm md:text-base">
                          Pelajari lebih jauh
