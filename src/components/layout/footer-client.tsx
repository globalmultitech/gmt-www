  ...
  return (
    <footer className="bg-primary text-primary-foreground">
      <div className="container mx-auto px-4">
        {/* Top Footer Section */}
        <div className="py-16 md:py-20 grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4">
            <Logo companyName={settings.companyName} logoUrl={settings.logoUrl} variant="footer" />
            <p className="text-primary-foreground/80 mt-6 text-sm md:text-base">
              {settings.footerText}
            </p>
          ...
          <div className="lg:col-span-2">
            <h3 className="font-headline font-bold text-lg md:text-xl mb-6">Perusahaan</h3>
            <ul className="space-y-3">
              {footerLinks1.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-sm md:text-base text-primary-foreground/80 hover:text-primary-foreground transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
           ...
           <div className="lg:col-span-2">
            <h3 className="font-headline font-bold text-lg md:text-xl mb-6">Sumber Daya</h3>
            <ul className="space-y-3">
          ...
          <div className="lg:col-span-4">
              <h3 className="font-headline font-bold text-lg md:text-xl mb-6">Hubungi Kami</h3>
              <ul className="space-y-3 text-sm md:text-base">
                {contactInfo.map((info) => (
                  <li key={info.label} className="flex items-start">
                    <span className="font-semibold w-24 md:w-32 shrink-0">{info.label}</span>
                    {info.href ? (
                      <Link 
                        href={info.href} 
        ...
        <div className="py-6 border-t border-primary-foreground/20 text-xs md:text-sm text-primary-foreground/80">
